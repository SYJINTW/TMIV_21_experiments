...
def psnr_ssim_vmaf(camera_position, scene, QP, synthesizer, numGroup, numFrame, pose_trace_num):
    ...
    """
    os.system(f"mkdir ./psnr ./psnr/{synthesizer}")
    ...
    return df['psnr_cr'].mean(), df['psnr_cr'].std(), df['float_ssim'].mean(), df['float_ssim'].std(), df['vmaf'].mean(), df['vmaf'].std()
...
def main():
    ...
    # 1
    # camera_positions = np.array(["6x6", "9x4", "12x3", "18x2"])
    # scenes = np.array(['ArchVizInterior'])
    # QPs = np.array([44])
    # synthesizers = np.array(["AdditiveSynthesizer", "ViewWeightingSynthesizer"])
    # numGroups = np.array([1])
    # pose_traces = np.array([f'pose{i}' for i in range(10)])
    ...
    # 2
    # camera_positions = np.array(["6x6", "9x4", "12x3", "18x2"])
    # scenes = np.array(['LightroomInteriorDayLight'])
    # QPs = np.array([44])
    # synthesizers = np.array(["AdditiveSynthesizer", "ViewWeightingSynthesizer"])
    # numGroups = np.array([1])
    # pose_traces = np.array([f'pose{i}' for i in range(10)])
    ...
    for scene in scenes:
        for camera_position in camera_positions:
                ...
                for synthesizer in synthesizers:
                    for numGroup in numGroups:
                        for pose_trace_num in range(np.size(pose_traces)):
                            psnr_mean, psnr_std, ssim_mean, ssim_std, vmaf_mean, vmaf_std = psnr_ssim_vmaf(
                                camera_position, scene, QP, synthesizer, numGroup, numFrame, pose_trace_num)
                            # print('psnr(mean/std):', psnr_mean, psnr_std)
                            # print('ssim(mean/std):', ssim_mean, ssim_std)
                            # print('vmaf(mean/std):', vmaf_mean, vmaf_std)
                            TMIV_encoding_time, TMIV_decoding_time, x265_encoding_time, x265_decoding_time = codec_time(
                                camera_position, scene, QP, synthesizer, numGroup, numFrame, pose_trace_num)
                            # print(f'TMIV_encoding_time: {TMIV_encoding_time}')
                            # print(f'TMIV_decoding_time: {TMIV_decoding_time}')
                            # print(f'x265_encoding_time: {x265_encoding_time}')
                            # print(f'x265_decoding_time: {x265_decoding_time}')
                            bitrate = get_folder_size(camera_position, scene, QP, synthesizer, numGroup, numFrame, pose_trace_num)
            ...
            df.to_csv(f"./results/{camera_position}_{scene}.csv", index=False, header=False)
